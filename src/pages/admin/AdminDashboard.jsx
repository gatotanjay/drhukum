...
import { Plus, Pencil, Trash2, Loader2, FileText, Scale } from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  return (
    <>
      <Helmet>
        <title>Artikel & Blog - Admin DrHukum</title>
      </Helmet>

      <div className="flex items-center justify-between mb-6">
        <p className="text-gray-600">{posts.length} post total</p>
        <Link to="/admin/posts/new">
          <Button className="bg-admin-navy hover:bg-admin-navyLight">
      ...
      <div className="bg-white rounded-xl shadow overflow-hidden">
        {isLoading ? (
          <div className="flex items-center justify-center py-16">
            <Loader2 className="h-6 w-6 animate-spin text-admin-gold" />
          </div>
        ) : posts.length === 0 ? (
              ...
              {posts.map((post) => (
                <TableRow key={post.id}>
                  ...
                  <TableCell>
                    ...
                    {!!post.featured && (
                      <Badge variant="outline" className="ml-1.5 border-admin-gold/40 text-admin-goldDark">Featured</Badge>
                    )}
                  </TableCell>
